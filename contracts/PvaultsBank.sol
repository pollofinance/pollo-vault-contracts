...
contract PvaultsBank is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public timelock = address(0x076743E359f561554089951c1562E958345DdfA1);
    ...
    function initialize(
        uint256 _startBlock,
        address _pofi,
        uint256 _pofiRewardRate,
        address _pdo,
        uint256 _pdoRewardRate
        ...
        devFund = msg.sender;
        operator = msg.sender;
        addRewardPool(_pofi, _pofiRewardRate);
        addRewardPool(_pdo, _pdoRewardRate);
        emit Initialized(msg.sender, block.number);
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "PvaultsBank: caller is not the operator");
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "PvaultsBank: caller is not timelock");
        _;
    }

    modifier notInitialized() {
        require(!initialized, "PvaultsBank: already initialized");
        _;
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({want: _want, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, depositFeeBP: _depositFeeBP, withdrawFeeBP: _withdrawFeeBP, strategy: _strategy}));
    }
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerBlock) public nonReentrant onlyOperator {
        require(rewardPoolInfo.length <= 16, "PvaultsBank: Reward pool length > 16");
        massUpdatePools();
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime) external nonReentrant onlyOperator {
        require(_unstakingFrozenTime <= 7 days, "PvaultsBank: !safe - dont lock for too long");
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            if(pool.depositFeeBP > 0) {
                ...
                pool.want.safeTransfer(devFund, depositFee);
                uint256 depositAmount = _wantAmt.sub(depositFee);
                uint256 sharesAdded = IStrategy(poolInfo[_pid].strategy).deposit(msg.sender, depositAmount);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        require(user.shares > 0, "PvaultsBank: user.shares is 0");
        require(sharesTotal > 0, "PvaultsBank: sharesTotal is 0");
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                require(whitelisted[msg.sender] || block.timestamp >= unfrozenStakeTime(_pid, msg.sender), "PvaultsBank: frozen");

                if(pool.withdrawFeeBP > 0) {
                    uint256 withdrawFee = _wantAmt.mul(pool.withdrawFeeBP).div(10000);
                    pool.want.safeTransfer(devFund, withdrawFee);
                    uint256 withdrawAmount = _wantAmt.sub(withdrawFee);
                    pool.want.safeTransfer(address(msg.sender), withdrawAmount);
                } else {
    ...
    function setTimelock(address _timelock) external {
        require(msg.sender == timelock || (timelock == address(0) && msg.sender == operator), "PvaultsBank: !authorised");
        timelock = _timelock;
    }

    function setDevFund(address _devFund) external {
        require(msg.sender == devFund, "PvaultsBank: !authorised");
        devFund = _devFund;
    }
    ...
    function executeTransaction(
        ...
        require(success, "PvaultsBank::executeTransaction: Transaction execution reverted.");
