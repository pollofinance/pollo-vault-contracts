...
contract PvaultsStrategyStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    address public uniRouterAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PancakeSwap: Router
    ...
    address public timelock = address(0x076743E359f561554089951c1562E958345DdfA1); // 24h timelock
    ...
    uint256 public buyBackRate1 = 0; // 1.9%
    address public buyBackToken1 = address(0x461f6C9aE13a7daC7055C73fBF8daB529D667041); // BFI
    address public buyBackAddress1 = address(0x000000000000000000000000000000000000dEaD); // to burn

    uint256 public buyBackRate2 = 0; // 1%
    address public buyBackToken2 = address(0x5BCcFbd33873A5498F8406146868eDdd5E998962); // BDO
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "PvaultsStrategy: caller is not the operator");
        _;
    }

    modifier onlyStrategist() {
        require(strategist == msg.sender || operator == msg.sender, "PvaultsStrategy: caller is not the strategist");
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "PvaultsStrategy: caller is not timelock");
        _;
    }
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        require(_wantAmt > 0, "PvaultsStrategy: !_wantAmt");
    ...
    function earn() public whenNotPaused {
        require(isAutoComp, "PvaultsStrategy: !isAutoComp");
        require(!notPublic || isAuthorised(msg.sender), "PvaultsStrategy: !authorised");
    ...
    function setEntranceFeeFactor(uint256 _entranceFeeFactor) external onlyOperator {
        require(_entranceFeeFactor > entranceFeeFactorLL, "PvaultsStrategy: !safe - too low");
        require(_entranceFeeFactor <= entranceFeeFactorMax, "PvaultsStrategy: !safe - too high");
        entranceFeeFactor = _entranceFeeFactor;
    }

    function setControllerFee(uint256 _controllerFee) external onlyOperator {
        require(_controllerFee <= controllerFeeUL, "PvaultsStrategy: too high");
        controllerFee = _controllerFee;
    }

    function setBuyBackRate1(uint256 _buyBackRate1) external onlyOperator {
        require(buyBackRate1 <= buyBackRateUL, "PvaultsStrategy: too high");
        buyBackRate1 = _buyBackRate1;
    }

    function setBuyBackRate2(uint256 _buyBackRate2) external onlyOperator {
        require(buyBackRate2 <= buyBackRateUL, "PvaultsStrategy: too high");
    ...
    function executeTransaction(
        ...
        require(success, "PvaultsStrategy::executeTransaction: Transaction execution reverted.");
